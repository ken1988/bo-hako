...
class Util {
  ...
  //---------------------------------------------------
	...
	$j = 0;
		...
		$nest_items = array_reverse($nest_items);
		 $html .= "<h2>{$name}収支レポート</h2>\n<div>";	
		 foreach($nest_items as $nest_item){
		 ...
		 $html .= "<div style=\"float:left;\" id=\"{$j}\" class=\"show_table\"><table border=1  class=\"nest\"><caption>{$year}年</caption>\n<thead><tr><th>区分</th><th>金額</th></tr></thead>";

		for ($i = 1; $i <10; $i++){
			$key = $i * 1000;
			if (array_key_exists($key,$init->accountName) == TRUE){
				$accountDesc = $init->accountName[$key][1];
				$DC = $nest_item[$key]['DC'];
				$amount = Util::Rewriter2('money',$nest_item[$key]['amount']);
				if($DC == 'D'){
					$html.="<tr><th>{$accountDesc}</th><td>{$amount}{$init->unitMoney}</td></tr>";
					$Debit += $nest_item[$key]['amount'];
					if($key == '4000'){
						$str_Det = Util::Rewriter2('money',$Debit);
						$html.="<tr><th>総収入</th><td>{$str_Det}{$init->unitMoney}</td></tr>";
					}
				}else{
					$html.="<tr><th>{$accountDesc}</th><td>{$amount}{$init->unitMoney}</td></tr>";
					$Credit += $nest_item[$key]['amount'];
					if($key == '9000'){
						$str_Cre = Util::Rewriter2('money',$Credit);
						$html.="<tr><th>総支出</th><td>{$str_Cre}{$init->unitMoney}</td></tr>";
					}
				}
			}
		}
		$Total = $Debit - $Credit;
		$Total = Util::Rewriter2('money',$Total);
		$html .= "</table>";
		$html .= "収支：{$Total}{$init->unitMoney}\n</div>";
		$j++;
	}
	//$html .= "</div>";
	return $html;
	...
	$nest_items = array_reverse($nest_items);
	 ...
	 $Credit = 0;
		 ...
		 for ($i = 1; $i <10; $i++){
		 	$key = $i * 1000;
		 	if(array_key_exists($key,$init->accountName) == TRUE){
		 		$accounts[] = $nest_item[$key]['amount']*10;
		 		if($nest_item[$key]['DC'] == 'D'){
			 		$Debit += $nest_item[$key]['amount']*10;
			 	}else{
			 		$Credit += $nest_item[$key]['amount']*10;	
			 	}
		 	}
		 }
	 if($Debit > $Credit){

...
class Log extends LogIO {
  ...
  // 砲弾自動製造
  function MakeAuto($id, $name, $num) {
    global $init;
    $this->secret("<A href=\"{$GLOBALS['THIS_FILE']}?Sight={$id}\">{$init->tagName_}{$name}{$init->_tagName}</A>で<strong>{$num}{$init->unitShell}</strong>の砲弾が生産されました。",$id);
  }
  // 資源採取
  ...
  function senkanAttack($id, $name, $lName, $point, $tname, $tPoint) {
    global $init;
    $this->out("<A href=\"{$GLOBALS['THIS_FILE']}?Sight={$id}\">{$init->tagName_}{$name}</A>{$point}{$init->_tagName}の<strong>{$lName}</strong>が多弾頭ミサイルを発射し、{$tPoint}の<strong>{$tname}</strong>に命中しました。",$id, $tId);
  }
  // 海軍、海賊・揚陸部隊を撃沈
  ...
  function msMonMoney($tId, $mName, $value) {
    global $init;
    $this->out("<strong>{$mName}</strong>の残骸には、<strong>{$value}{$init->unitMoney}</strong>の値段が付きました。",$tId);
  }
  // 通常ミサイル、怪獣に命中、ダメージ
  ...
  // 政変
  function DesGov($id, $name) {
    global $init;
    $this->out("<A href=\"{$GLOBALS['THIS_FILE']}?Sight={$id}\">{$init->tagName_}{$name}</A>{$init->_tagName}で政権与党が交代しました。",$id);
  }
  //自国駐屯地報告
  function FBDeb($id, $name,$tid,$tname) {
    global $init;
    $this->out("<A href=\"{$GLOBALS['THIS_FILE']}?Sight={$id}\">{$init->tagName_}{$name}</A>{$init->_tagName}に{$init->tagComName_}我が軍{$init->_tagComName}が駐屯中。",$tid);
  }
  //揚陸艦デバッガー
  ...
  // 経済統計デバッガー
  function NESTDebuger($id,$money) {
    global $init;
    $this->out("金額:{$money}{$init->unitMoney}",$id);
  }
  // 汎用デバッガー
  function CommonDebuger($id,$varn, $varv) {
    global $init;
    $this->out("{$varn}：{$varv}",$id);
  }
}

...
class Make {
  ...
  function NewsMain($hako, $data) {
    ...
    $island = $hako->islands[$num];
    $name = $island['name'];
	$dataset = array("nationID"=>'',"category"=>'',"text"=>'',"turn"=>'',"author"=>'',"ip"=>'');
	...
	$dataset['author']	 = mb_convert_encoding($data['TEXT2']."/".$name,"UTF-8",  "UTF-8,SJIS");
	$dataset['ip']		 = getenv("REMOTE_ADDR");
	
    $island['news_point']--;
    $hako->islands[$num] = $island;
    
    // データの書き出し
    $this->NewsPost($dataset);
    $hako->writeIslandsFile();
  ...
  function Capitalname($hako, $data) {
    ...
    switch($type) {
    	...
    	case "Wiki_Link":
	    //Wikiリンクを更新
	    $island['wiki_link'] = htmlspecialchars($data['Wiki_Link']);
		$msg = "国情報へのリンク";
	    break;
    
    	case "Trade_Link":
    	//貿易情報リンクを更新
	    $island['trade_link'] = htmlspecialchars($data['Trade_Link']);
	    $msg = "貿易情報へのリンク";
	    break;
...
class Turn {
  ...
  function normalizeInt($value, $default = 0) {
    return (int)$this->normalizeNumber($value, $default);
  }

  function normalizeIslandDefaults(&$island) {
    $island['propaganda'] = $this->normalizeInt(isset($island['propaganda']) ? $island['propaganda'] : 0);
    $island['riot'] = $this->normalizeInt(isset($island['riot']) ? $island['riot'] : 0);
    $island['monstersend'] = $this->normalizeInt(isset($island['monstersend']) ? $island['monstersend'] : 0);
    $island['prepare2'] = $this->normalizeInt(isset($island['prepare2']) ? $island['prepare2'] : 0);
    $island['fsocst'] = isset($island['fsocst']) ? $island['fsocst'] : false;
    $island['percat'] = $this->normalizeInt(isset($island['percat']) ? $island['percat'] : 0);
    $island['ffactory'] = $this->normalizeInt(isset($island['ffactory']) ? $island['ffactory'] : 0);
    $island['tenki'] = $this->normalizeInt(isset($island['tenki']) ? $island['tenki'] : 1, 1);

    $island['eisei'] = (isset($island['eisei']) && is_array($island['eisei'])) ? $island['eisei'] : array();
    for($i = 0; $i < 6; $i++) {
      $island['eisei'][$i] = $this->normalizeInt(isset($island['eisei'][$i]) ? $island['eisei'][$i] : 0);
    }

    $island['present'] = (isset($island['present']) && is_array($island['present'])) ? $island['present'] : array('item' => 0, 'px' => 0, 'py' => 0);
    $island['present']['item'] = $this->normalizeInt(isset($island['present']['item']) ? $island['present']['item'] : 0);
    $island['present']['px'] = $this->normalizeInt(isset($island['present']['px']) ? $island['present']['px'] : 0);
    $island['present']['py'] = $this->normalizeInt(isset($island['present']['py']) ? $island['present']['py'] : 0);
    ...
    foreach($shipKeys as $shipKey) {
      $island['ship'][$shipKey] = $this->normalizeInt(isset($island['ship'][$shipKey]) ? $island['ship'][$shipKey] : 0);
    }
  }
  ...
  function turnMain(&$hako, $data) {
    ...
    for($i = 0; $i < $hako->islandNumber; $i++) {
	 if ($hako->islands[$order[$i]]['freeze'] == 1) {
			 ...
			 if ( $present['item'] == 0 ) {
				if ( $present['px'] != 0 ) {
					$island['money'] += $present['px'];
					$this->log->presentMoney($island['id'], $island['name'], $present['px']);
				}
				if ( $present['py'] != 0 ) {
					$island['food'] += $present['py'];
					$this->log->presentFood($island['id'], $island['name'], $present['py']);
				}
			  }
			 unset($present);
			 unset($island);
  ...
  function doCommand(&$hako, &$island) {
    ...
    case $init->comPrepare2:
      ...
      if($kind == $init->comPrepare2) {
        // 地ならし
        $island['prepare2'] = isset($island['prepare2']) ? $island['prepare2'] + 1 : 1;
	...
	case $init->comFBase:
      ...
      switch($kind){
		...
		// 目的の場所を国立公園にする
	      ...
	      case $init->comSFactory:
	        if($arg == 2){
			  	if ($landKind == $init->landSFactory || $landKind == $init->landMFactory || $landKind == $init->landFFactory) {
					//軍事工場or建材工場or畜産場だったら
					$this->log->landFail($id, $name, $comName, $landName, $point);
					break;
				} else {
    ...
    case $init->comDisuse:
      // 鉱山廃止
      $mmin = min($init->mineKind);
      $mmax = max($init->mineKind);
    ...
    case $init->comEiseiLzr:
      ...
      if($island['eisei'][5] > 0 || $island['eisei'][3] > 0) {
        // イレギュラーか軍事衛星がある場合
        if(($tL == $init->landSea) && ($tLv < 2)||($tL == $init->landStonemine)||
           ($tL == $init->landSteel)||($tL == $init->landCoal)||
           ($tL == $init->landUranium)||($tL == $init->landSilver)||
           ($tL == $init->landnMountain)||($tL == $init->landMountain)) {
          // 効果のない地形（海、鉱山、山）
    ...
    case $init->comSendMonster:
      ...
      $tIsland['monstersend'] = isset($tIsland['monstersend']) ? $tIsland['monstersend'] + 1 : 1;
      $hako->islands[$tn] = $tIsland;
	  }
      $island['money'] -= $cost;
      $island['steel'] += $stonecost;
    ...
    case $init->comMkResource:
      ...
      if($landKind >= 25) {
        ...
        $resourcedeposits = floor($landValue[$x][$y] / 10);
        $minelv = ($landValue[$x][$y] % 10) + 1;
        $landValue[$x][$y] -= $resourcedeposits * 10; // 埋蔵量0に戻す
		$value = Turn::comMining($landKind,$resourcedeposits);
    ...
    case $init->comShell:
	  ...
	  if ($container == 'money'){
	  	$nest_item = array ("id" => $id, "account" => "7000", "amount" => $value);
		array_push($island['nest'],$nest_item);

		$tnest_item = array("id" => $tIsland['id'], "account" => "3000", "amount" => $value);
		array_push($tIsland['nest'],$tnest_item);
	  }
	  ...
	  //港が存在しない場合returnMode = 1（ターン消費あり）
		$returnMode = 1;
	  }else{
	  //港が存在する場合returnMode = 0（ターン消費なし）
		$returnMode = 0;
      }
      break;
  //------------------------------輸送処理ここまで---------------------
...
//    $hako->islands[$hako->idToNumber[$id]] = $island;
  ...
  function doEachHex(&$hako, &$island) {
    ...
    } elseif(!empty($island['propaganda'])) {
      // 誘致活動中
      $addpop = 20;
      $addpop2 = 5;
    ...
    for($i = 0; $i < $init->pointNumber; $i++) {
      ...
      case $init->landMonument:
        ...
        if(($lv == 5) || ($lv == 6) || ($lv == 21) || ($lv == 24) || ($lv == 32)) {
          ...
          if(util::random(100) < $init->disHosaku) {
            // 豊作
      ...
      case $init->landSilver:
        ...
        if(Util::random(1000) < $init->disRoofFall){
          ...
          if($minelv == 1){
            ...
            $landValue[$x][$y] --;
			$minelv --;
			$minedeposits = ($minelv - 1) * 10; //各鉱山レベルの初期値に戻す
			$landValue[$x][$y] = $minedeposits * 10 + ($minelv - 1);
          }
        }

        // 鉱山埋蔵量書き込み
        $landValue[$x][$y] += ($minelv -  floor($minedeposits / $minelv * 0.5)) * 10;
		$comName = $init->comName[$init->comMkResource];
		$point = "($x, $y)";

      // 資源採掘
	  $done = False;
	  $cost = $init->comCost[$init->comMkResource];
	  if ($island['fuel'] >= $cost){
        if(($hako->islandTurn%5 == 0)&&($landKind == $init->landStonemine)) {
          $value = Turn::comMining($landKind,$minedeposits);
          $island['stone'] += $value;
          $this->log->Resource($id, $name, $comName, $value, $point, '石材を',$init->unitStone);
		  $done = True;
        ...
        if(Util::random(100) < 30) {
          ...
          if ($value > 0)
            ...
            if ($value > 0){
	  		...
	  		$nest_item = array ("id" => $id, "account" => $account, "amount" => $value);
			array_push($island['nest'],$nest_item);
          }
          break;
      ...
      case $init->landZorasu:
	   ...
	   // 動いた元の地形によりメッセージ
        ...
        //移動可能か？
        $rlanding = Turn::checkLand($land, $x, $y);
        if($rlanding != false){
        //陸地に隣接している。既に上陸可能
        	$flanding = True;
	        $tx = $rlanding[0];
       ...
       //移動中は不可視判定
       	    ...
       	    if($insShip > 0){
       	    	$landValue[$sx][$sy] = floor($landValue[$sx][$sy]/10)*10+1;
       	    }else{
       	    	$landValue[$sx][$sy] = floor($landValue[$sx][$sy]/10)*10;
       	    }
        }
        // もと居た位置を海に
        $land[$x][$y] = $init->landSea;
        $landValue[$x][$y] = 0;
        $monsterMove[$sx][$sy] = (isset($monsterMove[$x][$y]) ? $monsterMove[$x][$y] : 0) + 1;
      ...
      case $init->landMonster:
        ...
        if (($special & 0x20000) && (Util::random(100) < 10)) { // 分裂確率10%
          if ($island['monster'] < 10){
          // 分裂する怪獣
          // もと居た位置を怪獣に
...
//------ 船の移動
  ...
  function doIslandProcess($hako, &$island) {
	...
	if (!empty($island['propaganda'])) {
	      // パレード中
		$island['siji']  += 6;
	}

	$siji = Ceil($island['siji'] + $hapiness);
	if($siji > 100){
	...
	if($island['civwar'] > 0){
		if(($island['civwar'] > 20) && ($island['polit'] == 1) && Util::random(100) < 40){
		...
		if($hako->islandTurn % 36 == 0){
			//36ターン(1年間）に1度、内戦リスクを下げる
			$island['civwar'] -= 4;
		}
	}
    ...
    do{
      if((($r < ($init->disMonster * $island['area'])) &&//出現確率クリア
		  ...
		  ($isMons) || ($island['monstersend'] > 0 || $island['civwar'] > 20))//プレゼントor怪獣派遣or上陸or内戦リスク
		  {
        // 怪獣出現
        // 種類を決める
    ...
    // 噴火判定
    if (($island['mountain'] < $init->haveMountain) && (Util::random(1000) < $init->disEruption)
  ...
  //労1：商品2.5、労1：商品1.5、労2：商品：1
		...
		if($island['indnum'] == 2){
          ...
          if($pgoods > 0){
            ...
            if($island['indnum'] == 1){
            	$shnum = ceil($sgoods/100);//重工業は砲弾を副産物として生産
            	$island['shell'] += $shnum;
            	$this->log->MakeAuto($id, $name, $shnum);
            }
          }
          if($island['fuel'] > (int)($wmarket * 0.2)) {
            ...
            if($island['goods'] > 0 ){
              ...
              $nest_item = array ("id" => $id, "account" => "1000", "amount" => $smoney);
			  array_push($island['nest'], $nest_item);
            }
          } else {
            // 不足メッセージ
...
if($island['ffactory']> 0){
	...
	// 観光収入
	$smoney = round($island['spop'] * 0.3 *(40 + $island['invest'] * 0.7) / 100);
	$island['money'] += $smoney;
    $nest_item = array ("id" => $id, "account" => "2000", "amount" => $smoney);
	array_push($island['nest'], $nest_item);
	...
	$smoney = round($island['cmente']);
	$island['money'] -= $smoney;
    $nest_item = array ("id" => $id,"account" => "6000", "amount" =>$smoney);
	array_push($island['nest'], $nest_item);
	}

	//社会保障費
	if ($island['money'] < $mentesoc){
		$this->log->Starve($id, $name, "社会保障費");
		$island['fsocst'] = True;
	}else{
	$island['money'] -= $mentesoc;
    $nest_item = array ("id" => $id,"account" =>"6000", "amount" =>$mentesoc);
	array_push($island['nest'], $nest_item);
	}
    ...
    if(($island['ship']['viking'] > 0) && (Util::random(1000) < $init->disRobViking)){
        if(!($island['money'] < $init->disVikingMinMoney) || !($island['food'] < $init->disVikingMinFood)){
            ...
            $vMoney = min($vMoney, $island['money']);
            $vFood = min($vFood, $island['food']);
            $island['money'] -= $vMoney;
            $island['food'] -= $vFood;
            
            $nest_item = array ("id" => $id,"account" => "9000", "amount" => $vMoney);
			array_push($island['nest'], $nest_item);
        }
    }
    if($island['money'] < 0) $island['money'] = 0;
  ...
  function estimate(&$island) {
	...
	$cmente = ceil($capitals * $services * $crowed / 100)+$bport*100+$mbase;
	...
	if (($m11 >= 1)&&($polit == 0)){
		...
		$hapiness += min($m11*3, 12);
	}
	if ($park >= 1){
		//遊園地1つにつき幸福度＋4（最大8）
		$hapiness += min($park*4, 8);
	}
	if ($hatuden > 0){
		//発電所がある場合幸福度＋8
		$hapiness += 8;
  ...
  public static function countAround($land, $x, $y, $kind, $range) {
    ...
    for($i = 0; $i < $range; $i++) {
      ...
      if(($sx < 0) || ($sx >= $init->islandSize) ||
        ...
        // 範囲内の場合
        if(isset($land[$sx][$sy]) && $land[$sx][$sy] == $kind) {
          $count++;
        }
      }
    }
    ...
    for($i = 0; $i < $range; $i++) {
        ...
        if(($sx < 0) || ($sx >= $init->islandSize) ||
           ($sy < 0) || ($sy >= $init->islandSize)) {
            ...
            // 範囲内の場合
            if(isset($land[$sx][$sy], $landValue[$sx][$sy]) &&
               ($land[$sx][$sy] == $init->landProcity) && ($landValue[$sx][$sy] >= $lv)) {
              // 防災都市は以上
               $count++;
            } elseif(isset($land[$sx][$sy], $landValue[$sx][$sy]) &&
                     $land[$sx][$sy] == $kind && $landValue[$sx][$sy] == $lv) {
               $count++;
            }
        }
    }
...
public static function comMining($kind,$resdep){
	...
	$min_value = $init->miningValue[$kind][1] * $resdep;
	$n_value = Util::random($n_value);

	$res_value = max($n_value, $min_value);
...
public static function AutoNews($id,$name,$turn,$text,$category){
	...
	$dataset['text']	 = $text;
	$dataset['turn']	 = $turn;
	$dataset['gamedate'] = Util::MKCal($turn,2);

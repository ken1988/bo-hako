...
<div ID="Loginbox">
	<div ID="login">
		...
		<input type="radio" name="DEVELOPEMODE" value="java" id="java" $radio2><label for="java">JS版</label><br>
...
if (($islandListStart != 1) || ($islandListSentinel != $hako->islandNumber)) {
  for ($i = 1; $i <= $hako->islandNumber ; $i += $init->islandListRange) {
    ...
    for($i = $islandListStart; $i < $islandListSentinel ; $i++) {
	  ...
	  $indad1 = "<IMG SRC=\"dash_borad/{$indpic}\" width=\"25\" height=\"25\" title=\"{$Tokka}\" ALT=\"{$Tokka}\">";
	  $happiad = "";
	  if($hapiness > 81) {
	  ...
	  $nprop ="[Developed]";
		...
		$edcad = "";
		if ($edinv > 0){
			$edcad .="<IMG SRC=\"dash_borad/edc.png\" width=\"23\" height=\"23\" title=\"教育指数：{$edinv}\" ALT=\"教育指数：{$edinv}\"><br/> {$edinv}";
		}

		$socad = "";
		$socad .="<IMG SRC=\"dash_borad/socsec.png\" width=\"23\" height=\"23\" title=\"社会保障指数：{$soclv}\" ALT=\"社会保障指数：{$soclv}\"><br/> {$soclv}";
		...
		$indad = "<IMG SRC=\"{$indad}\">";
			...
			$politad ="<IMG SRC=\"dash_borad/poli{$polit}.png\" width=\"25\" height=\"25\" title=\"{$poltype}\" ALT=\"{$poltype}\">";
      ...
      print "<tr>\n";
	  print "<td class=\"prevnat\" colspan=\"10\">{$init->tagTH_}<ul id=\"t{$id}\">
	  		<li class=\"IndD\">連絡<IMG SRC=\"dash_borad/comment.png\" width=\"23\" height=\"23\">
	  		<div class=\"databox\"><p id=\"comm\">{$comment}</p></div></a></li>
			<li class=\"IndD\">産業<br />{$indad1}
			<div class=\"databox\"><p>{$Tokka}  失業率:{$unemployed}<br/>公務員:{$service} 農業:{$farm} 工業:{$factory} 商業:{$market} {$indad}</p></div></a></li>
			<li class=\"IndD\">資源<IMG SRC=\"dash_borad/oil.png\" width=\"23\" height=\"23\">
			<div  class=\"databox\"><table><tr><td>食料{$mTmp1}</td><td>木材{$mTmp7}</td><td>石材{$mTmp8}</td><td>銀{$mTmp11}</td>
			...
			<a href=\"{$news_link}\">報道機関<IMG SRC=\"dash_borad/newspaper.png\" width=\"30\" height=\"30\" ALT=\"報道機関\"></a></p></div></a></li>
	  </ul>{$init->_tagTH}</td>\n";
      print "</tr>\n";
	  print "<tr>\n";
	  print "<td></td>\n";
...
END;
  ...
  print <<<END
  <form action="{$GLOBALS['THIS_FILE']}" method="post">
  国名<select name="ISLANDID" id="island_id">$hako->islandList</select><br>
  表示期間<input type="text" name="from_year" id="from_year" size="4">年～<input type="text" name="to_year" id="to_year" size="4">年<br>
  <input type="hidden" name="max_year" value="{$year}">
  <input type="hidden" name="mode" value="nest">
  <input type="submit" name="nest_display" id="nest_display" value="表示">
  </form>
  
  <div id="show_table">{$nest_html}
  <div id="show_graph" style="float:left;"></div></div>
  <hr style="clear:both;">
END;
...
END;
    ...
    $this->islandMap($hako, $island, $data['mobile'], 0);
    // 他の国へ
    print <<<END
...
END;
  ...
  // $mode = 1 -- ミサイル基地なども表示
  //---------------------------------------------------
  function islandMap($hako, $island, $mobile, $mode = 0) {
    global $init;
    $land      = $island['land'];
...
<td>
<input type="checkbox" name="conf" id="conf" value="ok">確認済
</td>
<td>
...
END;
    for($i = 0; $i < $init->regTMax; $i++) {
		if(!empty($regT[$i])){
		  ...
		  $nacanl = "";
		  $cancel = "";
		  $nallow = "<IMG SRC=\"accept.png\" ALT=\"許可\">";
		  $value = $value * $init->comCost[$kind];
		  $value = abs($value);
		  $unit  = $init->allunit[$kind - 71];
		  $kind = $init->comName[$kind];
		  $target = $tisland['name'];
		  if(($hako->islandTurn - $turn < $init->regTTerm) && ($hako->islandTurn != $turn)){
		  	$nacanl = "<IMG SRC=\"cancel.png\" ALT=\"不可\">";
		  }else{
		  	$cancel = "<input type=\"radio\" name=\"NUMBER\" value=\"{$i}\">";
		  }
	      if($j > $max){
  ...
  // 収支出力
  //---------------------------------------------------
   function BalanceOutput($hako, $island) {
   global $init;
   $j = 0;
   $year = Util::MKCal($hako->islandTurn,1);
   $o_year = $year - 2;
...
END;
    $this->islandMap($hako, $island,$data['mobile'], 1);    // 国の地図、所有者モード
    print <<<END
</td>
...
END;
    ...
    //国の地図
    $this->islandMap($hako, $island,$data['mobile'],2);
...
function cominput(theForm, x, k, z) {
 str = theForm.AMOUNT.value;
 if (str == ""){
 	str = "0";
 }
 var tmpe = str.match(/[0-9]+/g);
 if (tmpe!=str){
...
<tr valign="top">
<td $init->bgInputCell>
<form action="{$GLOBALS['THIS_FILE']}" method="post" name="InputPlan" id="InputPlan">
<input type="hidden" name="mode" value="command">
